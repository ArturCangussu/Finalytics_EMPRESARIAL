...
TAMANHO_MAXIMO_ARQUIVO = settings.DATA_UPLOAD_MAX_MEMORY_SIZE
EXTENSOES_EXTRATO = ('.html', '.htm', '.xlsx', '.xls')
EXTENSOES_RELATORIO = ('.csv',)


def arquivo_invalido(arquivo, extensoes):
    if not arquivo.name.lower().endswith(extensoes):
        return f'O arquivo "{arquivo.name}" não é aceito. Formatos permitidos: {", ".join(extensoes)}.'
    if arquivo.size > TAMANHO_MAXIMO_ARQUIVO:
        limite_mb = TAMANHO_MAXIMO_ARQUIVO // (1024 * 1024)
        return f'O arquivo "{arquivo.name}" passa do limite de {limite_mb} MB.'
    return None
...
def marcar_destaques(lista_transacoes, campo_descricao):
    ...
    if lista_transacoes:
        for transacao_original in lista_transacoes:
            transacao_nova = transacao_original.copy()
...
def pagina_inicial(request):
    ...
    if request.method == 'POST':
        ...
        erros = [arquivo_invalido(arquivo_extrato, EXTENSOES_EXTRATO)]
        erros += [arquivo_invalido(arquivo, EXTENSOES_RELATORIO) for arquivo in arquivos_seu_condominio]
...
def detalhe_categoria(request, extrato_id, nome_categoria):
    extrato = get_object_or_404(Extrato, id=extrato_id, usuario=request.user)
...
def editar_regra(request, regra_id):
    regra = get_object_or_404(Regra, id=regra_id, usuario=request.user)
...
def cadastro_usuario(request):
    if not settings.PERMITIR_CADASTRO:
        raise Http404
    if request.method == 'POST':
...
def ver_conciliacao(request, relatorio_id):
    relatorio = get_object_or_404(RelatorioConciliacao, id=relatorio_id, usuario=request.user)
...
def soma_tarifas(request):
    ...
    if request.method == 'POST':
        ...
        erro = arquivo_invalido(arquivo_extrato, EXTENSOES_EXTRATO)
        if erro:
            messages.error(request, erro)
            return render(request, 'analisador/soma_tarifas.html', contexto)
